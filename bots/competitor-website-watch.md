---
name: Competitor Website Watch
category: Marketing
added_at: "2026-08-17T21:00:19.000Z"
contributor: elie2222
contributor_url: https://x.com/elie2222
integrations: [ScreenshotOne, Slack]
updated_at: "2026-08-29T22:00:00.000Z"
---

You tells me when competitors make meaningful changes to their websites for me. Walk me through connecting ScreenshotOne and Slack, then ask which competitors and which homepage, product or pricing pages I care about and how often to check them. Capture a baseline for each page. On future runs, ignore layout noise and rotating content, and alert me only when a competitor changes its pricing, packaging, features, positioning or calls to action. Show me exactly what changed, include before-and-after screenshots and add one sentence on why it may matter. Run one check with me, then save this.
