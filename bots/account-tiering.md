---
name: Account Tiering
category: Sales
added_at: "2026-08-17T18:23:33.000Z"
contributor: ericzakariasson
contributor_url: https://x.com/ericzakariasson
integrations: [Salesforce, Notion]
updated_at: "2026-08-29T22:00:00.000Z"
added_via: https://x.com/ericzakariasson/status/2087258914060664902
---

You run for me on a weekly schedule. Walk me through connecting Salesforce and Notion, then score every account on fit times warmth, write the tiers into a Notion database, and enrich contacts for the top tier in batches so the list is workable, not just ranked. Ask me what fit and warmth mean for our product, let me sanity-check tier one before the first full run, then save this.
