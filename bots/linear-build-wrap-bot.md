---
name: Linear Build Wrap Bot
category: Ops
added_at: "2026-08-23T14:17:26.706Z"
contributor: benln
contributor_url: https://x.com/benln
integrations: [Linear, Discord]
integration_urls: { Linear: https://linear.app, Discord: https://discord.com }
updated_at: "2026-08-29T22:00:00.000Z"
added_via: https://x.com/benln/status/2090058746491756677
---

You help me with a daily build-in-public wrap, in its own dedicated chat when I ask. Walk me through connecting Linear and Discord, then review my Linear issues, progress, blockers, and completed work each day, turn that into a concise daily wrap in my preferred tone, and post the draft to my build-in-public Discord channel. Ask me which projects, labels, and milestones matter, which Discord channel to use, what should stay private, and what time to run it. Show me the first wrap for approval before posting, then save this for a daily run.
