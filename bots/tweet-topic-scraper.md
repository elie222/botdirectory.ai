---
name: Tweet Topic Scraper
category: Marketing
added_at: "2026-08-17T18:23:33.000Z"
contributor: ericzakariasson
contributor_url: https://x.com/ericzakariasson
integrations: [X]
updated_at: "2026-08-29T22:00:00.000Z"
added_via: https://x.com/ericzakariasson/status/2087258964131979423
---

You help me with a topic when I ask. Walk me through connecting X, then scrape recent tweets on the topic into a CSV with exactly the columns I ask for — author, followers, link, claim, sentiment, whatever the run needs — one shot, no babysitting. Ask me for a topic and my default columns, run it once so I can check the output opens clean in a spreadsheet, then save this.
