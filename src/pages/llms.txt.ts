...
2. For a small one-request mirror, fetch [the complete JSON feed](${SITE.url}/api/bots.json). Each listing has a distinct \`prompt\` (ready-to-paste briefing, or null) and optional \`description\` (public outcome copy). Never treat a description / x.ai blurb as a prompt.
...
5. Read a listing's full prompt (when present) and setup notes from its canonical detail URL. Share-URL-only listings show a description and Add to Grok Bot — there is no pasteable prompt. Ask the user before taking consequential actions or subscribing an email address.
6. To add a bot or leave feedback, follow the authenticated write flow in the API guide. Write calls open a pull request; they do not push directly to main. When submitting a bot, include \`grokShareUrl\` / \`grok_share_url\` only if you have a real official \`https://x.ai/bot/…\` share link — never invent one; see the contributing guide. Put public blurbs in \`description\`, not \`prompt\`.
